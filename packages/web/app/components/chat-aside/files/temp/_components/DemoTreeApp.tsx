...
export function DemoTreeApp() {
  const [loaded, setLoaded] = React.useState(false);
  const [preloadedData, setPreloadedData] = React.useState<any>(null);
  const [prerenderedHTMLByPath, setPrerenderedHTMLByPath] =
  ...
  React.useEffect(() => {
    async function load() {
      ...
      const htmlByPath = {
        dark: Object.fromEntries(
          preloadedEntries.map(
            ([path, variants]) => [path, variants.dark] as const,
          ),
        ),
      ...
      setPreloadedData(treePreloadedData);
      setPrerenderedHTMLByPath(htmlByPath);
      setLoaded(true);
    }
    load();
  }, []);

  if (!loaded || !preloadedData || !prerenderedHTMLByPath) {
    return <div>Loading tree demo...</div>;
  }
  ...
  return (
    <DemoTreeAppClient
      ...
      treePreloadedData={preloadedData}
    />
  );
}
