import spriteContent from '@/app/assets/file-types/sprite.svg?raw';

import { FILE_TYPE_ICON_IDS } from './file-type-icon-ids';
import { getLanguageFromExtension } from './tool-helpers';
...
const selectVariantIconName = (
  ...
  if (variant === 'light') {
    const lightName = iconName.endsWith('_light')
      ? iconName
      : `${iconName}_light`;
...
export const getFileTypeIconHref = (
  ...
  const baseIconName = FILE_TYPE_ICON_IDS.has(resolvedBaseIconName)
    ? resolvedBaseIconName
    : fallbackIconName;
  const iconName = selectVariantIconName(
    baseIconName,
