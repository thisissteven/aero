...
const localLogoModules = import.meta.glob<string>(
...
const resolveProviderLogoSrc = (
  providerId: string | null | undefined,
): string | null => {
  const candidates = buildLogoCandidates(providerId);
  const localResolvedId =
    candidates.find((candidate) => LOCAL_PROVIDER_LOGO_MAP.has(candidate)) ??
    null;
  const localLogoSrc = localResolvedId
    ? (LOCAL_PROVIDER_LOGO_MAP.get(localResolvedId) ?? null)
    : null;
  if (localLogoSrc) {
    return localLogoSrc;
  }
  ...
  return remoteResolvedId
    ? `https://models.dev/logos/${remoteResolvedId}.svg`
    : null;
};
...
export function useProviderLogo(
  ...
  const handleError = useCallback(() => {
    setSource((current) =>
      current === 'local' && hasLocalLogo ? 'remote' : 'none',
    );
  }, [hasLocalLogo]);
